...
**Dias 1 a 14 — concluídos.** O núcleo do Estoca está publicado em `https://estoca-erp.vercel.app`, com a API em `https://estoca-api.onrender.com` e PostgreSQL no Neon. Bootstrap, login, catálogo e movimentações foram validados em produção; o fallback sem cookie passou no WebKit 26.5 em viewport de iPhone, e os dois workflows de limpeza passaram via `workflow_dispatch`. O Dia 14 adicionou o fechamento do estoque com valor armazenado, unidades, categorias ativas e fila de reposição, validado visualmente em desktop e mobile. O gráfico foi cortado dentro do timebox porque a sandbox nasce com saldos zerados e a visualização começaria sem informação útil; nenhuma interface incompleta foi deixada visível. O README final registra arquitetura, decisões, execução local, verificações, hospedagem e próximos passos.
